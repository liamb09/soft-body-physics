...
public:
    ...
    void addShape (vector<Point> points, const bool &fixed = false);
    // void addShape (vector<Point>* points, const bool &fixed = false);
    ...
    void addLinkedShape (vector<vector<Point>> pointSets, vector<vector<pair<int, int>>> links, const vector<bool> &fixed);
    ...
    // void addGridShape (const float &x, const float &y, const float &width, const float &height, const float &numRows, const float &numColumns, const float &m = 5, const bool &fixed = false);
