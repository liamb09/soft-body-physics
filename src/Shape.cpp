	...
	if (!fixed) {
		...
		for (int i = 0; i < this->points.size()-1; i++) {
			for (int j = i+1; j < this->points.size(); j++) {
				springs.emplace_back(&(this->points[i]), &(this->points[j]), 750, 30); // 750 30
			}
		}
	} else {
		// fix each point
...
void Shape::resolveVelocities (Point &l1, Point &l2, Point &point, const float &ELASTICITY) {

	if (abs(l1.x-l2.x) > 0.01) { // avoid taking slope of vertical line
		// Calculates the intersect of the given line and the line with the slope perpendicular to the given line going through the given point.
		// Then, calculates the distance between that intersection and the given point.
		slope = (l2.y - l1.y)/(l2.x - l1.x);
		xInterception = (point.y*slope + point.x - l1.y*slope + l1.x*slope*slope)/(slope*slope + 1);
		...
		xInterception = l1.x;
		yInterception = point.y;
	}

	// If both are fixed (ex: wall touches ground) then exit
	if (l1.fixed && l2.fixed && point.fixed) return;

	// collision normal, represented by the vector between l1 and l2 that has been rotated 90 degrees and normalized
	Vector2D normal(l1.y-l2.y, l2.x-l1.x);
	normal.normalize();

	// If only the side is fixed
	if (l1.fixed && l2.fixed && !point.fixed) {
		...
		point.v = point.v - (1+sqrt(ELASTICITY))*(normal.dot(point.v))*normal;
		
		return;
	}

	// If only the point is fixed
	if (!l1.fixed && !l2.fixed && point.fixed) {
		// move side to point
		...
		// update side velocity
		l1.v = l1.v - (1+sqrt(ELASTICITY))*(normal.dot(l1.v))*normal;
		l2.v = l2.v - (1+sqrt(ELASTICITY))*(normal.dot(l2.v))*normal;
...
void Shape::handleCircleToCircleCollision (Point &a, Point &b, const float &ELASTICITY) {
	Vector2D normal(a.x-b.x, a.y-b.y);
	normal.normalize();

	// initial velocities of each point on normal
	float vAN = normal.dot(a.v);
	float vBN = normal.dot(b.v);
	...
	a.v = sqrt(ELASTICITY)*(a.v - vAN*normal + vANF*normal);
	b.v = sqrt(ELASTICITY)*(b.v - vBN*normal + vBNF*normal);
}

Point Shape::determineVirtualPoint(Point& c, Point& s1, Point& s2) {
	Vector2D normal(s1.y-s2.y, s2.x-s1.x);
	normal.normalize();
	Vector2D toVirtualPoint = normal.dot(c.v)/abs(normal.dot(c.v))*(c.m + s1.m + s2.m)*normal;
	return Point(c.x + toVirtualPoint.x, c.y + toVirtualPoint.y,
		(s1.x*s1.m + s2.x*s2.m)/(s1.m+s2.m), (s1.y*s1.m + s2.y*s2.m)/(s1.m+s2.m), s1.m+s2.m
	);
}